...
def _ensure_hide_index_html(path: Path) -> None:
    ...
    snippet = (
        ...
        "const cleanPath=window.location.pathname.slice(0,-'.html'.length);"
        "window.history.replaceState({},'',cleanPath+window.location.search+window.location.hash);"
        "}\n"
    )
    if "</script>" in text:
...
def _canonicalize_cohort_routes(cohort_dir: Path) -> None:
    ...
    # Remove directory-based routes from the previous attempt to make extensionless URLs.
    for old_dir in ("ch01", "ch02", "ch03", "ch04", "ch05", "ch06", "full"):
        shutil.rmtree(cohort_dir / old_dir, ignore_errors=True)
...
def _rewrite_cohort_links(path: Path, target_dir: Path) -> None:
    ...
    cohort_index_href = _relative_href(path.parent, cohort_root / "index.html")
    route_targets.append((r"(?:cohort/cohort_report\.html|cohort/index\.html|cohort/|cohort/cohort\.html)", cohort_index_href))
    for legacy_name, canonical_name in COHORT_CANONICAL_MAP.items():
        canonical_href = _relative_href(path.parent, cohort_root / canonical_name)
    ...
    full_href = _relative_href(path.parent, cohort_root / COHORT_FULL_CANONICAL_NAME)
...
def _rewrite_sessions_tree_relative_links(path: Path, target_dir: Path) -> None:
    ...
    if not path.exists():
        return
    # No-op for the flat sessions browser layout (sessions.html at atlas root).
    return
    text = path.read_text(encoding="utf-8")
